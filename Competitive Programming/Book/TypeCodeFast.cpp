...
// using global variables is a bad software engineering pratice, 
...
double matching(int bitmask){	// DP state = bitmask
	...
	if(bitmask == target)			// all students are already mathed
	...
	for(p1=0; p1<2*N; p1++)
		if(!(bitmask & (1 << p1)))
			break;
									// find the first bit taht is off
	for(p2=p1+1; p2<2*N; p2++)		// then, try ti natcg p1
