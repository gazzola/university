...
* Pior Caso: 	O(n^log7)
* Caso medio: 	O(n^log7)
* Melhor Caso:	O(n^log7)
*
* Memoria usada: 
* O(n*n) para matriz
* O(n*n) para resultado
*
* Algoritmo de Strassen retirado de: 
* https://github.com/MartinThoma/matrix-multiplication/
* Nao muito eficiente pois esta alocando mem dinamicamente
*/
...
void fillMatrix(vvi &mat){
	for(int i=0; i<MAX; i++)
		for(int j=0; j<MAX; j++)
			mat[i][j] =(rand()%MAX)-(rand()%MAX);
}



void squareMatrixMultiply(vvi &matA, vvi &matB, vvi &matR, int tam){
	for(int i=0; i<tam; i++){
		for(int j=0; j<tam; j++){
			matR[i][j] = 0;
			for(int k=0; k<tam; k++)
...
void sum(vvi &A, vvi &B, vvi &C, int tam){
	int i, j;
	for(i=0; i<tam; i++)
		for(j=0; j<tam; j++)
			C[i][j] = A[i][j] + B[i][j];
...
void subtract(vvi &A, vvi &B, vvi &C, int tam){
	int i, j;
	for(i=0; i<tam; i++)
		for(j=0; j<tam; j++)
			C[i][j] = A[i][j] - B[i][j];
...
void strassenR(vvi &A, vvi &B, vvi &C, int tam){
	
	if(tam <= leafsize){
		squareMatrixMultiply(A, B, C, tam);
		return;
	}
...
void strassen(vvi &A, vvi &B, vvi &C, unsigned int n){
	
	//unsigned int n = MAX;
	unsigned int m = nextPowerOfTwo(n);
	vector<int> inner(m);

	vvi APrep(m, inner), BPrep(m, inner), CPrep(m, inner);
...
int main(){
	...
	//squareMatrixMultiply(matA, matB, matR, MAX);
	strassen(matA, matB, matR, MAX);
	printMatrix(matR);
