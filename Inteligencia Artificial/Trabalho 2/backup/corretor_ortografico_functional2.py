...
class corretor_ortografico:
	...
	def print_resultado(self):
		...
		print("Levenshtein: Prob. da errada `%s` em relação a mais provável `%s`! [%.2f%%]" % (self.palavra_errada, self.palavra_provavel, prob_leve))
		print("Dice's     : Prob. da errada `%s` em relação a mais provável `%s`! [%.2f%%]" % (self.palavra_errada, self.palavra_provavel, prob_dice))
		...
		print("Levenshtein: Prob. da mais provável `%s` em relação a correta `%s`: [%.2f%%]" % (self.palavra_provavel, self.palavra_correta, prob_leve))
		print("Dice's     : Prob. da mais provável `%s` em relação a correta `%s`: [%.2f%%]" % (self.palavra_provavel, self.palavra_correta, prob_dice))
...
def main(arquivo_texto, arquivo_palavras):
	...
	# 	exemplo do livro / atencao: valores estao sendo normalizados por padrao:
	#	pl = {'v':0.5, 'f':0.5}
