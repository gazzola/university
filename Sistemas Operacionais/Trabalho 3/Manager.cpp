...
Manager<T>::Manager(double tclock){
	...
	this->tclock = tclock;
	this->interHandler = InterruptHandler<Interrupt>::getInstance();
...
void Manager<T>::executeProcess(int iterations){
	...
	while(!this->stateQueue[EXECUTANDO].front().alreadyExecuted()){
		...
		if(this->block()){
			...
			int qtdInters = (rand()%5)+1;
			int idInter;

			for(int k=0; k<qtdInters; k++)
				idInter = this->interHandler->addInter(this->interHandler->getTypoByRand(), rand()%4, (rand()%19)+1);

			bool tratado = this->interHandler->handle(idInter);
